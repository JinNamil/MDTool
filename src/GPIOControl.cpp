...
//#if 0
    ...
    ret = gpioSet(GPIOPINNAME_MS500_RST_CH1);
    if ( ret < 0 )
    {
        DBG_ERR("error!!!");
        return -1;
    }
	ret = digitalRead(pinNumber[GPIOPINNAME_MS500_RST_CH1]);

    /* sleep 0.5 sec */
    usleep(50*1000);

    ret = gpioReset(GPIOPINNAME_MS500_RST_CH1);
    if ( ret < 0 )
    {
        DBG_ERR("error!!!");
        return -1;
...
//    int ret = -1;
//    
//    for ( int i = SOCKET_CH1; i < SOCKET_CH2; i++ )
//    {
...
int GPIOControl::BootSocket(void)
{
	/* 0.1 only */
    int ret = -1;

	ret = digitalRead(pinNumber[GPIOPINNAME_MS500_RST_CH2]);
	DBG_LOG("Before Boot[0]: %d", ret);

	/* sleep 0.05 sec */
	usleep(50*1000);

	ret = gpioReset((eGPIOPINNAME)(GPIOPINNAME_MS500_RST_CH2));
	if ( ret < 0 )
	{
		DBG_ERR("error!!!");
		return -1;
...
int GPIOControl::ResetLED(void)
{
	...
	ret = digitalRead(pinNumber[GPIOPINNAME_MS500_CH1_LEDR]);
	if(ret == 0)
	{
		DBG_LOG("CH1 Red LED Off");
		ret = gpioSet((eGPIOPINNAME)GPIOPINNAME_MS500_CH1_LEDR);
		if ( ret < 0 )
		{
			DBG_ERR("error!!!");
			return -1;
	...
	ret = digitalRead(pinNumber[GPIOPINNAME_MS500_CH1_LEDG]);
	if(ret == 0)
	{
		DBG_LOG("CH1 Green LED Off");
		ret = gpioSet((eGPIOPINNAME)GPIOPINNAME_MS500_CH1_LEDG);
		if ( ret < 0 )
		{
			DBG_ERR("error!!!");
			return -1;
